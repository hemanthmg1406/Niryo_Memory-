...
def collect_cards_to_stacks(robot):
    ...
    for slot_id in ALL_SQUARE_IDS:
        ...
        # Use the base stack pose directly for dropping
        new_stack_pose = stack_info["pose"][:]

        print(f"[MOVE] Collecting {slot_id} → {stack_info['id']}")
        ...
        try:
            ...
            # B. Move to Stack and Drop
            safe_move(robot, new_stack_pose)
...
def place_initial_cards(robot):
    ...
    stack_pick_counts = {stack_id: 0 for stack_id in STACKS_DATA.keys()}

    for stack_id in STACKS_DATA.keys():
        source_picks_sequence.extend([stack_id] * 5)
    ...
    for i in range(len(source_picks_sequence)):
        ...
        if stack_pick_counts[source_id] < 2:
            stack_pick_pose[2] = 0.055
        else:
            stack_pick_pose[2] = 0.05
        
        stack_pick_counts[source_id] += 1
